...
function Get-WtReswTextValues {
    ...
    $vals = @()
    try {
        $repoRoot = Get-ItRepoRoot
        $resDirs = @(
            (Join-Path $repoRoot 'src\cascadia\TerminalApp\Resources'),
            (Join-Path $repoRoot 'src\cascadia\TerminalSettingsEditor\Resources')
...
function Get-WtaLocalizedTextRegex {
    ...
    $result = $null
    try {
        $localeDir = Join-Path (Get-ItRepoRoot) 'tools\wta\locales'
...
function Resolve-AgentOwnerTabId {
    ...
    try {
        ...
        $tabEvent = Wait-WtEvent -Listener $listener -TimeoutSec 15 -Predicate {
            $_.method -eq 'vt_sequence' -and
            "$($_.params.pane_id)" -eq $OwnerPaneSessionId -and
            $_.params.tab_id
        }
        [string]$tabEvent.params.tab_id
    }
    finally {
