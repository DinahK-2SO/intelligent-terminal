# Install/verify local TDD runtime dependencies and import the standalone module.
#   pwsh -File local-tdd-kit/bootstrap.ps1            # install missing test tools
#   pwsh -File local-tdd-kit/bootstrap.ps1 -Check     # verify only
...
$haveGit = Test-Tool 'Git' 'git'
$haveCargo = Test-Tool 'Rust/Cargo (build only)' 'cargo'
...
if (-not $Check) {
    if (-not $haveWinapp) {
        if (-not (Get-Command winget -ErrorAction SilentlyContinue)) {
            throw 'Windows App CLI is missing and winget is unavailable. Install Microsoft.WinAppCli manually.'
        }
...
# Import the module only after all runtime prerequisites pass.
if (-not $psOk -or -not $haveGit -or -not $haveWinapp -or -not $pester) {
    Write-Host '[fail] Required local test runtime prerequisites are missing.' -ForegroundColor Red
    exit 1
}
...
if (-not $haveCargo) { Write-Host '[warn] Cargo is required by Invoke-BuildDeploy.ps1 but not by test-only workflows.' -ForegroundColor Yellow }
