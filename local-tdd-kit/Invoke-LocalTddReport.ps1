...
.DESCRIPTION
    Wraps Invoke-Pester and writes, into -OutDir (default: local-tdd-kit/artifacts/):
      - report.html        Self-contained HTML report (open in a browser):
            ...
            - results.xml        NUnit XML for local analysis or optional manual import
      - summary.md         Human-readable Markdown summary (same per-failure blocks)
        Prints the same failure blocks to the console and returns a script exit code
    (0 = all passed, 1 = any failure).

.EXAMPLE
    pwsh -File local-tdd-kit/Invoke-LocalTddReport.ps1 -Tag Unit
    pwsh -File local-tdd-kit/Invoke-LocalTddReport.ps1 -Path local-tdd-kit/selftests/ItE2E.Input.Tests.ps1
#>
[CmdletBinding()]
param(
    [string[]]$Path = @("$PSScriptRoot/selftests"),
    [string[]]$Tag,
