...
Describe 'Feature §1/§6 Settings editor UI (opened via Ctrl+, accelerator)' -Tag 'Feature' -Skip:(-not $script:Ready) {
    ...
    It 'Token usage toggle defaults off and persists when enabled' {
        ...
        Test-UiElementExists -App $script:app -Selector 'ShowTokenUsageToggle' -TimeoutSec 8 |
            Should -BeTrue -Because 'Settings > Agents must expose the token usage preference'
        (Get-UiElement -App $script:app -Selector 'ShowTokenUsageToggle').toggleState |
            Should -Be 'off' -Because 'token usage must be hidden by default'

        Invoke-UiElement -App $script:app -Selector 'ShowTokenUsageToggle' | Out-Null
        Invoke-UiElement -App $script:app -Selector 'SaveButton' | Out-Null
        Wait-Until -TimeoutSec 8 -Because 'the Settings toggle to persist showTokenUsage=true' -Condition {
            (Get-WtSettingsObject -App $script:app).showTokenUsage -eq $true
        } | Out-Null
        Assert-Setting -App $script:app -Key 'showTokenUsage' -Value $true
    }
}
